...
func saveMetadata(target string, added bool, pathFile string, oldUserPath string) error {
	...
	// Also attempt to write metadata into invoking user's data dir when run under
	// sudo so that the original user can later run `krnr status` / `krnr uninstall`.
	_ = copyMetadataToSudoUser(b, target)
	return nil
}
...
// copyMetadataToSudoUser writes a copy of the install metadata into the
// invoking user's data dir when the installer runs under sudo. This keeps
// status/uninstall usable by the original user.
func copyMetadataToSudoUser(b []byte, target string) error {
	su := os.Getenv("SUDO_USER")
	if su == "" {
		return nil
	}
	// allow tests to override the SUDO user's home
	...
	// If running as root, make the sudo user own the metadata file so later
	// non-root `krnr status` / `krnr uninstall` can read it.
	if os.Geteuid() == 0 {
...
// ensureOwnershipForSudoUser chowns the installed binary and its directory to
// the invoking user when the installer runs as root but targets a user
// install. Errors are ignored — best-effort behavior.
func ensureOwnershipForSudoUser(targetPath string, opts Options) {
...
// applyMetadataToStatus loads saved metadata (if present) and adjusts the
// provided Status to reflect recorded AddedToPath and the recorded target.
func applyMetadataToStatus(st *Status) {
	...
	// fallback: mark metadata presence if metadata file exists in current data dir
...
func executeInstallNow(opts Options, targetPath string) ([]string, error) {
	...
	ensureOwnershipForSudoUser(targetPath, opts)
